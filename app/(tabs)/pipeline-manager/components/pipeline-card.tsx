...
export function PipelineCard({ item, onDelete, onRetry, color }: Props) {
      ...
      <GlassCard intensity={25} style={styles.card}>
        ...
        <View style={styles.statusRow}>
          ...
          {item.status === "processing" && (
            <Text style={[styles.progress, { color: color.textSecondary }]}>
              {getProgressLabel(item.completed_chunks, item.total_chunks)}
            </Text>
          )}
        </View>
