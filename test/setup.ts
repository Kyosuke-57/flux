...
vi.mock("react-native", () => {
  ...
  const renderSlot = (slot: any) => {
    if (typeof slot === "function") return h(slot);
    return slot;
  };
  ...
  return {
    ...
    FlatList: ({
      ...
      if (ListHeaderComponent) {
        const el = renderSlot(ListHeaderComponent);
        if (el) children.push(h("View", { key: "header" }, el));
      }
      if (data && data.length > 0) {
        data.forEach((item: any, i: number) => {
          const key = keyExtractor?.(item, i) ?? String(i);
          children.push(
            h("View", { key }, renderItem({ item, index: i, separators: {} })),
          );
        });
      } else if (ListEmptyComponent) {
        const el = renderSlot(ListEmptyComponent);
        if (el) children.push(h("View", { key: "empty" }, el));
      }
